...
app.get("/ping/:cid", async (req, res) => {
  ...
  if (trafficText === "IPHONE-712"){
    redirectLink = `${redirectLink}`.replace("{click_id}", `${req.params.cid}`)
    redirectLink = `https://myphoneispretty.com?popclick=${req.params.cid}&r=` + encodeURIComponent(redirectLink)
  ...
  if (trafficText === "KETO-OS"){
    ...
    try {
      ...
      redirectLink = redirectLink +  `&shipping_firstname=${kFirstName}&shipping_lastname=${kLastName}&shipping_city=${kcity}&shipping_state=${kstate}&shipping_zipcode=${kzip}&shipping_email=${kemail}&shipping_phone=${kphone}&shipping_country=US&shipping_address=${kaddress}`
    } catch (error) {
      console.log(error)
    }
